<div id="kt_form_create_product">
    <form id="kt_create_product_form" class="form d-flex flex-column flex-lg-row" action="{{ route('product.type.update', ['productId' => $productFind->uuid, 'typeId' => $productTypeField->id]) }}" enctype="multipart/form-data" class="form" method="POST">
        @csrf
        @method('PUT')
                                    ...
                                    <div class="mb-10 fv-row">
                                        ...
                                        @php
                                            ...
                                            if (in_array($field, ["floater_kubikasi", "motor_kubikasi", "floater_kg", "motor_kg"])) {
                                                ...
                                                if (count($parts) >= 3) {
                                                    // Gabungkan elemen kedua dan ketiga, dan tambahkan unit jika ada
                                                    $combined = strtolower($parts[1] . ' ' . $parts[2]);
                                                    $formattedField = ucwords($parts[1] . ' ' . $parts[2]);
                                                    $unit = isset($units[$combined]) ? ' ' . $units[$combined] : '';
                                                    $formattedField .= $unit;
                                                } else {
