    ...
    return {
        ...
        */
        ...
        submitForm: function () {
            // *Insert Or Update*
            // *INSERT*
            let url = route("targets.store");
            const targetId = $("#target-id").val();
            const targetMonth = $("#target-month").val();
            const targetAmount = $("#target-amount").val();
            const targetYear = $("#target-year").val();
            const targetPId = $("#p-id").val();

            let formData = new FormData();
            // *UPDATE*
            if (targetId) {
                url = route("targets.update", { id: targetId });
            ...
            formData.append("pic_id", targetPId);
            ...
            axios
                ...
                .catch(({ response }) => {
                    ...
                    if (response.data.errors) {
                        $("#error-target-month").text(
                            response.data.errors.month
                        );
                        $("#error-target-amount").text(
                            response.data.errors.target
                        );
                    }
                })
                .finally(() => {
