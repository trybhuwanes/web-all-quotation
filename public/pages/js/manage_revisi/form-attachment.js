...
var KTFileUploadAttach = (function () {
    ...
    return {
        init: function () {
            ...
            if (!dropzoneInstance) {
                ...
                dropzoneInstance = new Dropzone("#kt_modal_attachment_files_upload", {
                    url: route("storage-attachment-dropzone-pdf"), // Ganti dengan URL upload yang sesuai
                    paramName: "file",
        ...
        submitForm: function () {
            ...
            if (attachmentId) {
                url = route("order-attachment", { id: attachmentId });
                formData.append("_method", "put");
                formData.append("id", attachmentId);
            } else {
                url = route("order-attachment"); // Route untuk create jika belum ada attachmentId
            }
...
$(document).on("click", "#delete-attachment-btn", function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            axios.delete(route("order-attachment.destroy", { id: orderId }))
                .then(({ data }) => {
                    if (data.success) {
                        Swal.fire({
                            text: data.message,
