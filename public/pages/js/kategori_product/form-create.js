    ...
    return {
        ...
        */
        ...
        submitForm: function () {
            // *Insert Or Update*
            // *INSERT*
            let url = route("kategori-product.store");
            const categoryId = $("#category-id").val();
            const nameKategori = $("#category-name").val();
            const photoLogo = $("#file-logo");
            const deskrpsiKategori = $("#category-description").val();

            let formData = new FormData();
            // *UPDATE*
            if (categoryId) {
                url = route("kategori-product.update", { id: categoryId });
                formData.append("_method", "put");
                formData.append("id", categoryId);
            }
            formData.append("nama_kategori", nameKategori);
            formData.append("deskripsi", deskrpsiKategori);

            if (photoLogo.get(0).files.length !== 0) {
                formData.append("logo", photoLogo[0].files[0]);
            }
